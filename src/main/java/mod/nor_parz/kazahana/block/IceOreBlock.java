...
import mod.nor_parz.kazahana.world.dimension.MafuyuDimension;
import mod.nor_parz.kazahana.itemgroup.TabMaterialsItemGroup;
import mod.nor_parz.kazahana.item.IceIngotItem;
...
public class IceOreBlock extends KazahanaModElements.ModElement {
	@ObjectHolder("kazahana:ice_ore")
	public static final Block block = null;
	public IceOreBlock(KazahanaModElements instance) {
		super(instance, 10);
	}
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(IceIngotItem.block, (int) (1)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("kazahana:mafuyu_biome")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
			biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Biome.createDecoratedFeature(new OreFeature(OreFeatureConfig::deserialize) {
				...
				public boolean place(IWorld world, ChunkGenerator generator, Random rand, BlockPos pos, OreFeatureConfig config) {
					...
					if (dimensionType == MafuyuDimension.type)
