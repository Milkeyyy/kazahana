...
package mod.nor_parz.kazahana.world.dimension;
...
public class MafuyuDimension extends KazahanaModElements.ModElement {
	@ObjectHolder("kazahana:mafuyu")
	public static final ModDimension dimension = null;
	@ObjectHolder("kazahana:mafuyu_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("mafuyu"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("kazahana:mafuyu")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("kazahana:mafuyu"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("kazahana:mafuyu"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new MafuyuItem().setRegistryName("mafuyu"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = MafuyuDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("mafuyu_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "mafuyu_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = MafuyuDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, GlisteningIceBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
